...
## Author: katernet
## Version 1.8.1
## License: GPL, see darkmode.LICENSE
...
darkMode() {
	case $1 in
		off)
			...
			# VS Code Theme
			if ls ~/.dotfiles/vscode/settings.json >/dev/null 2>&1; then # VS Code Dotfile Setup
				sed -i '' 's/Bluloco Dark/Bluloco Light/g' ~/.dotfiles/vscode/settings.json
			fi
		...
		on)
			...
			# VS Code Theme
			if ls ~/.dotfiles/vscode/settings.json >/dev/null 2>&1; then # VS Code Dotfile Setup
				sed -i '' 's/Bluloco Light/Bluloco Dark/g' ~/.dotfiles/vscode/settings.json
			fi
